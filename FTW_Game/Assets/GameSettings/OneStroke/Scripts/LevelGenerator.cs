...
namespace FTW.Game.OneStroke
{
    public class LevelGenerator : MonoBehaviour
    {
        ...
        private void Update()
        {
            print(Input.GetMouseButtonDown(0).ToString());
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                ...
                int id = spawnId++;
                Vector3 spawnPos = new(mousePos.x, mousePos.y, 0);
                Vector4 point = new(mousePos.x, mousePos.y, 0, id);
            ...
            {
                if (startSpawnPoint == null || endSpawnPoint == null)
                {
                    return;
                }

                if (startSpawnPoint.Id == endSpawnPoint.Id)
                {
                    return;
                }

                Vector2Int normal = new(startSpawnPoint.Id, endSpawnPoint.Id);
                Vector2Int reversed = new(normal.y, normal.x);
                if (edges.ContainsKey(normal))
                {
                    return;
                }
